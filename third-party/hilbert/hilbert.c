...
int hilbert_ieee_cmp(unsigned nDims, double const *c1, double const *c2) {
  ...
  return hilbert_cmp_work(nDims, 8, 64, max, max + 53, (char const *)c1,
                          (char const *)c2, rotation, bits, index, getIEEEBits);
}
...
unsigned hilbert_ieee_box_vtx(unsigned nDims, int findMin, double *c1,
  ...
  return hilbert_box_vtx_work(nDims, 8, 64, findMin, max, max + 53, (char *)c1,
...
unsigned hilbert_box_pt(unsigned nDims, unsigned nBytes, unsigned nBits,
  ...
  return hilbert_box_pt_work(nDims, nBytes, nBits, findMin, 0, nBits,
                             (char *)c1, (char *)c2, 0, bits, bits, getIntBits,
                             propogateIntBits);
}
    ...
    for (r = 0; r < nPrints; ++r) {
      ...
      if (hilbert_cmp(nDims, sizeof(coord[0]), nBits, coord, coord1) != 0)
        printf("cmp r = 0x%0*x; r1 = 0x%0*x\n", (nDims * nBits + 3) / 4,
               (unsigned)r, (nDims * nBits + 3) / 4, (unsigned)r1);
    }
  }
  return 0;
}
...
int main() {
  ...
  hilbert_box_vtx(nDims, sizeof(unsigned), 8 * sizeof(unsigned), 1, cornerlo,
                  work);
  ...
  hilbert_box_vtx(nDims, sizeof(unsigned), 8 * sizeof(unsigned), 0, work,
                  cornerhi);
...
int main() {
  ...
  hilbert_box_pt(nDims, sizeof(unsigned), 8 * sizeof(unsigned), 1, pointlo,
                 work);
  ...
  hilbert_box_pt(nDims, sizeof(unsigned), 8 * sizeof(unsigned), 0, work,
                 pointhi);
